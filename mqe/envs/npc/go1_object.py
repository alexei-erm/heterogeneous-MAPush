...
class Go1Object(Go1):
    ...
    def _prepare_npc(self):
        ...
        for rigid_shape_prop in rigid_shape_props_asset:
            rigid_shape_prop.friction = friction_range[0] + torch.rand(1).item() * (friction_range[1] - friction_range[0])
        ...
        if getattr(self.cfg.env, "num_obs", 0) != 0:
            ...
            self.obs_asset_npc = self.gym.load_asset(self.sim, obs_asset_root_npc, obs_asset_file_npc, obs_asset_options_npc)
        ...
        if getattr(self.cfg.goal, "sequential_goal_pos",False):
            ...
            self.base_init_state_npc[1, :7] = torch.tensor(init_target_pos, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _create_npc(self, env_handle, env_id):
        ...
        if getattr(self.cfg.asset, "file_npc_final", None) != None:
            npc_handle = self.gym.create_actor(env_handle, self._asset_npc1, self.start_pose_npc, self.cfg.asset.name_npc_final, env_id, not self.cfg.asset.npc_collision_final, 0)
            npc_handles.append(npc_handle)
        # create obstacle
        num_obs = getattr(self.cfg.env, "num_obs", 0)
        for _ in range(num_obs):
            npc_handle = self.gym.create_actor(env_handle, self.obs_asset_npc, self.start_pose_npc, self.cfg.asset.obs_name_npc, env_id, not self.cfg.asset.obs_npc_collision, 0)
            npc_handles.append(npc_handle)
        return npc_handles