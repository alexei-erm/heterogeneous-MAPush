...
class mqe_openrl_wrapper(gym.Wrapper):
    def __init__(self, env):
        self.env = env
        if getattr(self.env.cfg.env, "hie", False):
            self.agent_num = 1
            self.action_space = spaces.Box(low=-10, high=10, shape=(2,), dtype=float)
        else:
    ...
    def step(self, actions, extra_data: Optional[Dict[str, Any]] = None, **kwargs):
        """Step all environments."""
        actions = torch.from_numpy(0.5 * actions).cuda().clip(-1, 1)
        
        # target_pos = kwargs.get("target_pos", None)
        # if target_pos is not None:
        #     obs, reward, termination, info = self.env.step(actions, next_target_pos=target_pos)
        # else:
    ...
    #     self.env.cfg.goal.received_final_pos = target_pos
    ...
    def batch_rewards(self, buffer):
        ...
        step_count = self.env.reward_buffer["step_count"]
        reward_dict = {"average_step_reward": 0}
...
def parse_arguments(parser, headless=False, no_graphics=False, custom_parameters=[]):
    ...
    parser.add_argument("--test_mode",type=str, default=None, help="Test mode for the environment")
...
def get_args():
    ...
    custom_parameters = [
        {"name": "--task", "type": str, "default": "go1push", "help": "Select task via name"},
