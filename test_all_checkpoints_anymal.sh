#!/bin/bash
# Test all checkpoints in a directory for heterogeneous Go1 + Anymal C

if [ "$#" -lt 1 ]; then
    echo "Usage: ./test_all_checkpoints_anymal.sh <checkpoints_dir> [num_episodes] [num_envs]"
    echo "Example: ./test_all_checkpoints_anymal.sh results/.../checkpoints 50 300"
    exit 1
fi
...
echo ""
echo "Testing heterogeneous: Go1 + Anymal C"
echo ""

# Output file
OUTPUT_FILE="$CKPT_DIR/test_results_anymal.txt"
echo "======================================================================" > "$OUTPUT_FILE"
echo "MAPush HAPPO Testing Results (Heterogeneous: Go1 + Anymal C)" >> "$OUTPUT_FILE"
echo "======================================================================" >> "$OUTPUT_FILE"
...
echo "$CHECKPOINTS" | while read CKPT_PATH; do
    ...
    # Run test directly with conda environment (avoids conda run libtinfo issue)
    OUTPUT=$(conda run -n mapush python HARL/harl_mapush/test.py \
        --checkpoint "$CKPT_PATH" \
        --mode calculator \
        --num_episodes "$NUM_EPISODES" \
        --num_envs "$NUM_ENVS" \
        --hetero_agent anymal_c 2>&1)
